...
module TUI
  module Markdown
    ...
    # resolving emphasis (`*`/`_`, doubled and tripled), inline code
    # (backtick-fenced), links (`[text](url)`), and backslash-escapes.
    # Deliberately non-nesting: the first opening delimiter's matching
    # closer ends that span outright, rather than tracking a delimiter
    # stack — covers the overwhelming majority of real-world Markdown
    # while keeping the scanner a single linear left-to-right pass with
    # no backtracking. An opening delimiter with no matching closer
    # anywhere ahead degrades to literal text (emitted as-is) rather than
    # consuming the rest of the line or raising, since a hand-rolled
    # scanner must never hang on malformed input.
    module Inline
      ESCAPABLE = "\\`*_[]"
